...
\ShortHeadings{Intelligence via ultrafilters}{S.\ Alexander}
...
or \emph{DLHA} for short, to distinguish it from various other agent notions.
...
\end{proof}
    ...
    the above-mentioned comparison with practical chess
...
environments with the \emph{intention}\footnote{That is, intention on the part of
the designer. We emphasize the intention part
of this argument because to assume that an environment can \emph{succeed} in
rewarding creativity would be begging the question.} of rewarding creativity, or
of rewarding pattern
recognition, or of rewarding other specific components of intelligence.
...
\citep{philebus}.
% Even Jesus of Nazareth speaks about the relationship
...
% related to intelligence. We suspect he would tear such answers to shreds.
...
% Our
% Section \ref{structuralpropertiessection} refers to and depends on the
% specific nature of the agent-environment model (Definition \ref{EnvironmentAndAgent}),
% whereas everything done in \citet{hutter2007} would work
% if the agent-environment model were replaced by \emph{any} arbitrary bounded function
% assigning real numbers (called \emph{total rewards}) to agent-environment pairs.
