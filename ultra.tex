...
\ShortHeadings{Structural properties of universal intelligence}{S.\ Alexander}

\author{\name Samuel Allen Alexander \email samuelallenalexander@gmail.com}

\title{Toward structural properties of universal intelligence}
...
\begin{abstract}
    ...
    numeric intelligence of one agent, how can we compare the relative intelligence
    of two agents? We propose an elegant answer based on
    the following insight:
    we can view agents as candidates in an election, whose voters are
...
point of view.}
(formalized
below). These agents' intelligence is measured in
various ways using real numbers (for example, by
means of infinite series involving Kolmogorov complexities).

Aside from quantifying the intelligence of individual agents, this field
of research has the potential to shed light on intelligence itself,
abstracted over many agents.
To that end, we intend to present a few
\emph{structural properties} of intelligence. What do I mean by ``structural
properties''? Roughly speaking, I mean the following: if I know something about the
intelligence of an agent $A$, a ``structural property'' is a theorem which allows me
...
particular agent, we will instead focus on the simpler problem of
how to determine whether one agent is more intelligent
than another agent\footnote{In more practical, less universal contexts, much work has
...
We will introduce an abstract intelligence measure
...
and other authors\footnote{For simplicity, we require both agent and environment to
...
than $B$ on that environment. We would defend this choice by pointing out that for any
