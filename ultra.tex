...
As long as there is any way to make sense of some two entities interacting with
...
but that comparison is surely quite meaningless. For example, an interface intended
to be used by robots could get hit by a tornado, causing it to output rewards in
response to its keys being randomly pressed by said tornado. Probably the tornado
would seem indistinguishable from a DLHA who ignores its environment and performs
actions according to a pseudo-random number generator. We conjecture that, except
